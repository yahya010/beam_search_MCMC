...
# from transformers import BeamScorer, BeamHypotheses, GPT2Tokenizer
...
class BeamSearchScorer(BeamScorer):
    ...
    # def calculate_inclusion_probabilities(self, W: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
    #     """
    #     Calculate inclusion probabilities for each candidate.
        
    #     Args:
    #         W: Dynamic programming table (K+1 x N+1)
    #         weights: Weights for each candidate (N) which is probs / (1 - probs)
        
    #     Returns:
    #         inclusion_probs: Inclusion probabilities for each candidate (N)
    #     """
    #     # import pdb; pdb.set_trace()
    #     K, N = W.shape[0] - 1, W.shape[1] - 1
    #     inclusion_probs = torch.zeros(N, device=W.device)
    #     epsilon = 1e-6
    #     for n in range(1, N + 1):
    #         inclusion_probs[n-1] = (weights[n-1] * W[K-1, n-1]) / (W[K, n] + epsilon)
    ...
    decoder_prompt_len: Optional[int] = 0,
        ...
        #next_beam_inclusion_probs = torch.zeros((batch_size, self.group_size), dtype=next_scores.dtype, device=device)

        
        if eos_token_id is not None and not isinstance(eos_token_id, torch.Tensor):
            eos_token_id = torch.tensor([eos_token_id] if isinstance(eos_token_id, int) else eos_token_id)
        total_estimated_bleu = 0.0
        for batch_idx in range(batch_size):
            ...
            # Calculate weights for CPSBS
            probs = torch.softmax(next_scores[batch_idx], dim=-1)
            mytokenizer = GPT2Tokenizer.from_pretrained("gpt2")
            decoded_tokens = mytokenizer.decode(next_tokens[batch_idx])
            decoded_input = mytokenizer.decode(input_ids[batch_idx])
            print("Decoded Input:", decoded_input)
            print("Decoded Tokens:", decoded_tokens)
            print("Probs:", probs)

            
            weights = probs / (1 - probs)  # w_n = p(y_n) / (1 - p(y_n))
            #weights = torch.log(probs) - torch.log1p(-probs)

            # Prepare for conditional Poisson sampling
            N = len(weights)
            K = self.group_size

            # Initialize W
            W = torch.zeros((K+1, N+1), device=device)
            W[0, :] = 1
            ...
            print("W Matrix:", W)
            #####using logprobs, maybe? TODO: check if this is correct or epsilon
            #weights = torch.log(probs) - torch.log1p(-probs)
            #W = torch.zeros((K+1, N+1), device=device)
            # W[0, :] = 0  # log(1) = 0
            # for n in range(1, N+1):
            #     for k in range(1, K+1):
            #         W[k, n] = torch.logaddexp(W[k, n-1], weights[n-1] + W[k-1, n-1])
            ###
            #inclusion_probs = self.calculate_inclusion_probabilities(W, weights)
            # conditional Poisson sampling
            sampled_indices = []
            sampled_inclusion_probs = []
            # Sampling in reverse order
            for n in range(N, 0, -1):
                k = K - len(sampled_indices)
                if k == 0:
                    break
                prob = (weights[n - 1] * W[k - 1, n - 1]) / (W[k, n] + 1e-10)  # TODO: WRONG, this isn't how to do inclusion probabilities
                if torch.rand(1, device=device).item() < prob.item():
                    sampled_indices.append(n - 1)
                    sampled_inclusion_probs.append(prob.item())


            # Ensure we have exactly K samples
            if len(sampled_indices) < K:
                remaining = K - len(sampled_indices)
                unsampled = list(set(range(N)) - set(sampled_indices))
                for idx in unsampled[:remaining]:
                    sampled_indices.append(idx)
                    # Approximate inclusion probability for unsampled items
                    prob = (weights[idx] * W[K - len(sampled_indices), idx]) / (W[K - len(sampled_indices) + 1, idx + 1] + 1e-10)
                    sampled_inclusion_probs.append(prob.item())

            # Convert to tensors
            sampled_indices = torch.tensor(sampled_indices, device=device)
            sampled_inclusion_probs = torch.tensor(sampled_inclusion_probs, device=device)

            # Initialize BLEU score estimation for this batch
            estimated_bleu = 0.0

            # Update beams and compute BLEU scores
            for i, idx in enumerate(sampled_indices):
                inclusion_prob = sampled_inclusion_probs[i]
                batch_beam_idx = batch_idx * self.group_size + next_indices[batch_idx][idx]

                # Decode the beam to text
                beam = input_ids[batch_beam_idx].tolist()
                # Append the new token
                beam.append(next_tokens[batch_idx][idx].item())
                # Decode using the tokenizer
                mytokenizer = GPT2Tokenizer.from_pretrained("gpt2")
                generated_text = mytokenizer.decode(beam, skip_special_tokens=True)

                # Compute BLEU score
                # You need to provide the actual reference text here
                reference_text = "Your reference text goes here."
                bleu_score = sentence_bleu([reference_text.split()], generated_text.split())

                # Update the estimated BLEU score using Horvitz-Thompson estimator
                estimated_bleu += bleu_score / (inclusion_prob + 1e-10)  # Add epsilon to avoid division by zero

                # Update beams as before
                if (eos_token_id is not None) and (next_tokens[batch_idx, idx].item() in eos_token_id):
                    ...
                    self._beam_hyps[batch_group_idx].add(
                        ...
                        next_scores[batch_idx, idx].item(),
                        beam_indices=beam_index,
                        generated_len=cur_len - decoder_prompt_len,
                    )
                else:
                    # Add next predicted token since it is not eos_token
                    next_beam_scores[batch_idx, i] = next_scores[batch_idx, idx]
                    next_beam_tokens[batch_idx, i] = next_tokens[batch_idx, idx]
                    next_beam_indices[batch_idx, i] = batch_beam_idx
                # next_beam_inclusion_probs[batch_idx, i] = inclusion_prob

            # Normalize the estimated BLEU score
            estimated_bleu /= K
            print(f"Estimated BLEU Score for batch {batch_idx}: {estimated_bleu}")
            total_estimated_bleu += estimated_bleu

            # Check if we are done to save a pad step if all(done)
            self._done[batch_group_idx] = self._done[batch_group_idx] or self._beam_hyps[batch_group_idx].is_done(
                next_scores[batch_idx].max().item(), cur_len, decoder_prompt_len
            )

        # Average BLEU score over all batches
        average_estimated_bleu = total_estimated_bleu / batch_size
        print(f"Average Estimated BLEU Score: {average_estimated_bleu}")

        # Print next beams for debugging
        print("Next Tokens:", next_beam_tokens)
        print("Next Scores:", next_beam_scores)
        print("Next Indices:", next_beam_indices)
        # print("Inclusion Probabilities:", next_beam_inclusion_probs)
        ...
        return UserDict(
            {
                ...
                # "next_beam_inclusion_probs": next_beam_inclusion_probs.view(-1),
            }
        )
