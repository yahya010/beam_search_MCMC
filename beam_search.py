...
class BeamSearchScorer(BeamScorer):
    ...
    decoder_prompt_len: Optional[int] = 0,
        ...
        for batch_idx in range(batch_size):
            ...
            probs = F.softmax(next_scores[batch_idx], dim=-1)

            #TODO: Implement Poisson Sampling, currently just using poisson weights of 1 and then multinomial sampling, which isn't needed
            
            # Generate Poisson weights (lambda can be changed randomness)
            poisson_weights = torch.poisson(torch.full_like(probs, 1.0))  # lambda = 1

            # Adjust probabilities by multiplying with Poisson weights
            adjusted_probs = probs * poisson_weights
            adjusted_probs = adjusted_probs / adjusted_probs.sum() 

            # Sample tokens based on their adjusted probabilities
            sampled_indices = torch.multinomial(adjusted_probs, num_samples=self.group_size, replacement=True)
            sampled_tokens = next_tokens[batch_idx][sampled_indices]
            sampled_scores = next_scores[batch_idx][sampled_indices]
            sampled_next_indices = next_indices[batch_idx][sampled_indices]

            for i in range(self.group_size):
                batch_beam_idx = batch_idx * self.group_size + sampled_next_indices[i]

                # add to generated hypotheses if end of sentence
                if (eos_token_id is not None) and (sampled_tokens[i].item() in eos_token_id):
                    ...
                    self._beam_hyps[batch_group_idx].add(
                        ...
                        sampled_scores[i].item(),
                        beam_indices=beam_index,
                        generated_len=cur_len - decoder_prompt_len,
                    )
                else:
                    # add next predicted token since it is not eos_token
                    next_beam_scores[batch_idx, beam_idx] = sampled_scores[i]
                    next_beam_tokens[batch_idx, beam_idx] = sampled_tokens[i]
        ...
        # next_tokens_text = [tokenizer.decode(tokens, skip_special_tokens=True) for tokens in next_tokens]
        # print("Next Tokens (Text):", next_tokens_text)
